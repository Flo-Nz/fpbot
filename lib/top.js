...
export const generateTextRatingButton = (username) => {
    const oropTextRatingButton = new ButtonBuilder()
        .setCustomId('textrating')
        .setStyle(ButtonStyle.Success)
        .setLabel(`${username}, tu y as joué ? Note le !`)
        .setDisabled(true);
...
export const getTopRated = async (onlyFP) => {
    try {
        const response = await apiClient({
            method: 'get',
            url: '/orop/top/rated',
            params: {
                onlyFP: onlyFP === 'firstplayer' ? true : false,
            },
        });
        const { data } = response;
        return data;
...
export const getTopSearched = async (withVideo) => {
    try {
        const response = await apiClient({
            method: 'get',
            url: '/orop/top/searched',
            params: {
                withVideo,
            },
        });
