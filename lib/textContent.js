import { bold, hyperlink, inlineCode, userMention } from 'discord.js';
import { fpEmojis, testEmojis } from './emojis.js';

const emojisLib = process.env.NODE_ENV === 'PROD' ? fpEmojis : testEmojis;

const generateRatingPrimaryContent = (created, updated) => {
    if (created) {
        return `Tu es le premier à noter ce jeu !\n`;
    }
    if (updated) {
        return `Tu as déjà noté ce jeu petit(e) coquin(e). Peut-être qu'Alzheimer te guette... Bref, j'ai mis à jour ta note.\n`;
    }
    return '';
};
const generateFpRatingContent = (fpRating) =>
    fpRating ? `La note de Yoël: ${emojisLib[fpRating]}\n` : '';
const generateDiscordRatingContent = (discordRating, discordOrop) => {
    let content;
    content = discordRating
        ? `La note moyenne du Discord: ${emojisLib[discordRating]} `
        : '';
    content =
        discordOrop?.ratings?.length > 0
            ? content + `(Noté ${discordOrop.ratings.length} fois) \n`
            : '\n';
    return content;
};
...
export const generateRatingReplyContent = (discordOrop, username, rating) => {
    ...
    const primaryContent = generateRatingPrimaryContent(
        discordOrop.created,
        discordOrop.updated
    );
    const fpContent = generateFpRatingContent(orop?.fpOrop?.rating);
    const discordContent = generateDiscordRatingContent(
        orop?.discordRating,
        orop?.discordOrop
    );
...
const generateOropRatingContent = (orop) => {
    ...
    return `${content} Le jeu a été recherché ${searchCount} fois.`;
};

export const generateOropContent = (title, orop, userId) => {
    const primaryContent = generateOropPrimaryContent(title, orop, userId);
