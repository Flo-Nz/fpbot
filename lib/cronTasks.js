...
const getAllOrops = async () => {
    try {
        ...
        console.log(('getAllOrops error', error));
    }
};

const getOneOrop = async (title, skipSearchInc) => {
    try {
        ...
        console.log('getOneOrop - error', error);
    }
};
...
export const oneDayOneGame = async (client) => {
    try {
        ...
        const channel = await client.channels.fetch(ratingChannelId);
